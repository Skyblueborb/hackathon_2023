...
def factor(year: int) -> float: # średnia pensja dla roku [year]
    ...
    return numCleaner((soup.select(".zus-cms>table>tbody>tr>td")[1::3][year-1950]).text)
calculate = lambda amount, fact: (-1 if amount < 0 else round((amount/fact)*CURRENT_PAY, 2))# (-1): error
